...
export function buildSubmitHeaderBlockPayload(
  ...
  const data = Web3EthAbi.encodeParameters(
    ...
    [
      ...
      utils.hexZeroPad(root, 32),
      utils.hexZeroPad(rewardsRoot, 32),
      MATIC_CHAIN_ID,
    ]
  );
...
export class CheckpointHelper {
  ...
  async submitCheckpoint(
    ...
    if (start > end) {
      throw new Error(
        `Invalid end block number for checkpoint { start: ${start} end: ${end} }`
      );
    }
    const tree = await buildBlockHeaderMerkle(
      provider,
      start,
      end,
      this.offset
