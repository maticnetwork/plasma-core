...
export class EthersAdapter implements IProviderAdapter {
  ...
  private buildBlockFromRpcData(blockData: any, transformTxs: boolean): Block {
    if (transformTxs) {
      blockData.transactions = blockData.transactions.map((t: any) =>
        this.transformRpcTx(t)
      );
    }
  ...
  async getBlockByHash(hash: string): Promise<Block> {
    const block: any = await this.provider.send("eth_getBlockByHash", [
      hash,
      true,
    ]);
  ...
  async getBlock(
    ...
    const block: any = await this.provider.send("eth_getBlockByNumber", [
      _number,
      includeTxObject,
    ]);
    block.number = number;
  ...
  async getTransactionReceipt(txHash: string): Promise<TransactionReceipt> {
    const txData = await this.provider.send("eth_getTransactionReceipt", [
      txHash,
    ]);
    return {
